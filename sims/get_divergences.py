...
def grid_samples(ts, n, m, prob=1.0, shrink=0.75):
    ...
    sample_inds = [[[] for _ in range(m)] for _ in range(n)]
    ...
    for ind in ts.individuals():
        if np.random.uniform() < prob:
            ...
            if (abs(x_diff) < shrink / 2) and (abs(y_diff) < shrink / 2):
                sample_inds[i][j].append(ind)
    # put these in random order
    for i in range(n):
        for j in range(m):
            np.random.shuffle(sample_inds[i][j])
            for ind in sample_inds[i][j]:
