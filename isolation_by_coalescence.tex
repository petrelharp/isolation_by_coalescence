...
especially in the presence of asymmetry.
...
\citet{slatkin1991inbreeding,rousset1997genetic},
...
is therefore an unbiased estimate of the mean time to most recent common ancestor,
...
equation \eqref{eqn:R_from_H} provides $n (n-1)/2$ informative equations.
...
\citet{epstein2008badtruth,myers2008learn,terhorst2015fundamental}.
...
but coalescence time to $i$ only requires lineages to leave $i$,
...
but in principle it still could be the case that the movement rates that give the best fit of commute time
...
produced by applying both inference methods with a noise level of 1/200.
...
as well as the popular resistance-based method EEMS \citep{petkova2016visualizing},
...
\citet{wakeley2012genealogies}, \citet{waples2009modelling}, or \citet{ralph2015empirical}.
...
\caption{
    ...
    All values are plotted against 
    mean coalescence times calculated using the first set of individuals from the first simulation.
    Circles show values calculated from a second (non-overlapping) set of individuals 
    from the same simulation:
    without sampling noise, all would fall on the $y=x$ line.
    The other symbols are calculated from two independent simulations:
    without process noise, these should deviate from $y=x$ no more than the circles.
    The mean percent absolute differences between each set of coalescence times
    and the reference values are
    1.8\% for the second set of individuals from the first simulation,
...
the original papers describing Circuitscape \citep{mcrae2006isolation} and EEMS \citep{petkova2016visualizing}
...
This tradeoff implies a high degree of unavoidable uncertainty.
Reproducible, reliable inference will likely require development of new inference methods
explicitly incorporating continuous geography.
...
hopefully obtaining a map at the best possible resolution supported by the data.
...
However, these methods requires fairly dense genotyping and a genetic map,
...
The landscape of many areas has changed substantially over this time, % \citep{pleistocene_mojave},
...
\plr{ERIK: also John, Hossein, Desi, ???}
