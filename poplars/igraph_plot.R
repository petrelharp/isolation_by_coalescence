# copied version of igraph::plot.igraph
#   that we can get the coordinates out of
my_plot_igraph <- function (x, axes = FALSE, add = FALSE, xlim = c(-1, 1), ylim = c(-1, 
    ...
    alpha = 1), mark.expand = 15, ...) 
{
    ...
    return(list(x=x, y=y, lc.x=lc.x, lc.y=lc.y))
}

environment(my_plot_igraph) <- asNamespace("igraph")
