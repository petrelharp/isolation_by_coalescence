...
Thanks to the five mostly anonymous reviewers for their many useful comments --
we've addressed all of them, which we think has made the paper more generally understandable
and suitable for this audience.
In particular, we have included analysis of an empirical dataset 
(which was fun, and produced good results),
revised the figures for increased legibility,
and added a substantial amount of additional explanation of the technical points.
We hope that discussion of the empirical results should help make interpretation
of the coalescent-based model more broadly understandable.
And, at the suggestion of reviewer 1,
we have changed the title.
We think this has all improved the paper substantially,
and hope you agree.

We have included detailed responses to all the reviewer's comments,
with page and line numbers that refer to the revised version of the manuscript.
We are also including a file with changes made since the first submission highlighted
(named \texttt{ibc-diff.pdf}).
Finally, we are including an additional pdf (named \texttt{isolation\_by\_coalescence.pdf})
that has the paper, the supplementary material, and the responses to reviewers all in one place
in which the page/line numbers in the reviews are hyperlinks 
...
We hope you agree we've addressed all four of these points.
...
\reply{
    True! We've tweaked the way the arrows are displayed,
    and think they look better now.
}
...
% but then we learned that this species likely expanded from Alaska, rather than from the south.)
...
\reply{
    ...
    Something along these lines might be useful for finer geographical resolutions, though.
    % Developing a way to reduce the number of parameters in a way similar to EEMS
...
\reply{
    Added parenthetical clarification. \revref
}
...
\reply{
    Added explanatory note about how to estimate noise. \revref
}
...
\reply{
    That's correct; we've reworded this to be more clear. \revref
}
...
\reply{
    We mean among individuals (and have added this). \revref
}
...
\reply{
    We're saying that it is possible to \emph{run} the method
    to have samples in each location --
    so it is easy, in principle, to include ``ghost'' populations.
...
\reply{
    We sympathize, and have increased the sizes of most of the labels.
    However, the x-axis labels of figures like Figure~\ref{fig:5x3b_post_coalvcom}
    cannot be made larger without overlapping, and we'd like to keep them.
...
\reply{
    A lengthy acronym (that we've added, as well as MAPS, later). \revref
}
...
\reply{
    It could, but this is the way we implemented it in the simulation.
}
...
\reply{
    ...
    The only downside to coalescence time inference is computational tractability
    (discussed at \llname{ll:approx}):
    otherwise, one can be swapped out for the other.
    In fact, \emph{any} method that uses resistance distances could in principle be 
...
\reply{
    ...
    that move independently according to $G$ until they coalesce,
    and so
    \begin{align*}
        C_{xz} &= dt + {} \\
                & \qquad
